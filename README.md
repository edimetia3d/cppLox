...
* [Optional] Prebuild LLVM
    * For LLVM is a huge project to download/build, when mlir_jit backend is not enabled, llvm project will not be
      needed.
    * If you want to build with the mlir_jit backend, run the `third_party/prepare_mlir.sh` to download/prebuild llvm.
...
2. `-DENABLE_MLIR_JIT_BACKEND=ON`. Enable the mlir_jit backend, OFF by default. Note that you may need to prebuild LLVM
   to
   use this option.

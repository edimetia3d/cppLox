...
public:
  ...
  template <ConcreteASTNode SubT, class... Args> static std::unique_ptr<SubT> Make(Args &&...args) {
    return std::unique_ptr<SubT>(new SubT(std::forward<Args>(args)...));
  }

  virtual void Accept(IASTNodeVisitor *visitor) = 0;

  template <ConcreteASTNode SubT> SubT *As() { return static_cast<SubT *>(this); }

  template <ConcreteASTNode SubT> SubT *DynAs() { return dynamic_cast<SubT *>(this); }
